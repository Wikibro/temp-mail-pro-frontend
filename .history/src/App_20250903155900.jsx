...
function TempMailApp() {
  ...
  const location = useLocation();
  const isAppPage = location.pathname === "/app";
  const currentUrl = typeof window !== 'undefined' ? `${window.location.origin}${location.pathname}${location.search}` : '';
  ...
  useEffect(() => {
    if (account) {
      localStorage.setItem(`inbox-${account.token}`, JSON.stringify(messages));
    }
  }, [messages, account]);
  ...
  }, [account]); // Only restart interval when account changes
